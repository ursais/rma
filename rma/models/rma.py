...
class Rma(models.Model):
    ...
    def _compute_delivery_picking_count(self):
        ...
        for record in self:
            delivery_picking_count = mapped_data.get(record.id, 0)
        record.delivery_picking_count = delivery_picking_count
    ...
    def _send_draft_email(self):
        ...
        for rma in self.filtered("company_id.send_rma_draft_confirmation"):
            rma_template_id = rma.company_id.rma_mail_draft_confirmation_template_id
            rma.with_context(
                force_send=True,
                mark_rma_as_sent=True,
                default_subtype_id=self.env.ref("rma.mt_rma_notification").id,
            ).message_mail_with_source(rma_template_id)

    def _send_confirmation_email(self):
        """Auto send notifications"""
        for rma in self.filtered(lambda p: p.company_id.send_rma_confirmation):
            rma_template_id = rma.company_id.rma_mail_confirmation_template_id
            rma.with_context(
                force_send=True,
                mark_rma_as_sent=True,
                default_subtype_id=self.env.ref("rma.mt_rma_notification").id,
    ...
    def _send_receipt_confirmation_email(self):
        ...
        for rma in self.filtered("company_id.send_rma_receipt_confirmation"):
            rma_template_id = (
                rma.company_id.rma_mail_receipt_confirmation_template_id
            )
            rma.with_context(
                force_send=True,
                mark_rma_as_sent=True,
    ...
    def create_return(self, scheduled_date, qty=None, uom=None):
        ...
        for rmas in grouped_rmas:
            ...
            for rma in rmas:
                ...
                picking_vals = picking_form._get_save_values()
